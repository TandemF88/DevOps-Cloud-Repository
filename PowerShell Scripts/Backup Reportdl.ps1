...
Process
{
    ...
    foreach ($vm in $vms) 
    {
        ...
        if ($recoveryVaultInfo.BackedUp -eq $true)
        {
            ...
            [array]$vmBackupVault = $backupVaults | Where-Object {$_.ID -eq $recoveryVaultInfo.VaultId}

            #$vmBackupVault = $recoveryVaultInfo.VaultId
            ...
            $backupItem = Get-AzRecoveryServicesBackupItem -Container $container[0] -WorkloadType AzureVM -VaultId $vmBackupVault.ID
        } #if ($recoveryVaultInfo.BackedUp -eq $true)
        else 
        {
...
{
    $vmBackupReport | Export-Csv C:\Temp\backupreport.csv
} #end