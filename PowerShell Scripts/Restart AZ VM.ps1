...
foreach ($server in $servers){
    ...
    #New-AzSnapshot -Snapshot $snapshot -SnapshotName "CTASK0304232Snapshot-$server" -ResourceGroupName $RG

    #cmdlet to restart each of the VM's
    Restart-AzVm -ResourceGroupName $RG -Name $server
    ...
    Add-Content C:\Temp\snapshotlist.txt "Restarted-$server,$RG"
